...
const  NSAID_NO_COEF = 0;
const  NSAID_ORAL_COEF  = 0.263;
const  NSAID_RECTAL_COEF  = 0.159;
...
function retrieveLatestInputs() {
    ...
    nSAIDuse = document.querySelector('input[name="nSAIDuse"]:checked').value;
}
...
function calcResult() {
    ...
    nSAIDuse_factor = (nSAIDuse == "nSAIDuseNo") ? (STONE_POSITION_LOWER_COEF) : ((nSAIDuse == "nSAIDuseOral") ? (NSAID_ORAL_COEF) : (NSAID_RECTAL_COEF));

    var xBeta = RESULT_BASE + gender_factor + neutrophil_factor + hydroureter_factor + hydronephrosis_factor + highTemp_factor + stoneSize_factor + stonePos_factor+ nSAIDuse_factor;
...
function helpTemperature() {
  infoBoxHeader.innerHTML = "Medical Expulsive Therapy";
  infoContent.innerHTML = "Use of i.e. Tamsulosin ";
  showInfoBox();
}
